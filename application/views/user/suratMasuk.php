<!-- Begin Page Content -->
<div class="container-fluid">

    <div class="row">
        <div class="col-9">
            <h1 style="font-size: 24px; color:black">Data <small style="font-size: 60%">Surat Masuk</small> </h1>
        </div>
        <div class="col-3">
            <button class="btn btn-primary float-right">
                <i class="fas fa-fw fa-plus"></i>
                Add Surat Masuk
            </button>
        </div>
    </div>

    <div class="card mx-auto mt-3">
        <div class="card-body">
            <div class="row">
                <div class="col-6">
                    <div class="form-group row">
                        <label for="inputPassword" class="col-sm-2 col-form-label">Show</label>
                        <div class="col-sm-3">
                            <select class="custom-select mr-sm-2" id="inlineFormCustomSelect">
                                <option selected>10</option>
                                <option value="1">One</option>
                                <option value="2">Two</option>
                                <option value="3">Three</option>
                            </select>
                        </div>
                        <span class="col-sm-3 col-form-label pl-0">entries</span>
                    </div>
                </div>

                <p>Date: <input type="text" id="datepicker"></p>
                <div class="col-6">
                    <div class="form-group row">
                        <label for="inputPassword" class="col-sm-6 text-right col-form-label">Search</label>
                        <div class="col-sm-6">
                            <input type="text" class="form-control">
                        </div>
                    </div>
                </div>
                <table class="table">
                    <thead>
                        <tr>
                            <th scope="col">#</th>
                            <th scope="col">Tanggal dan Jam</th>
                            <th scope="col">Nomor Surat</th>
                            <th scope="col">Pengirim</th>
                            <th scope="col">Disposisi</th>
                            <th scope="col">Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <th scope="row">1</th>
                            <td>2018-03-05 / 15:03:54</td>
                            <td> <a href="http://">201803/MAS/12-001</a> </td>
                            <td>DGT</td>
                            <td>Diterima Resepsionis</td>
                            <td>
                                <button class="btn btn-sm btn-primary"><i class="fas fa-fw fa-folder-open"></i></button>
                                <button class="btn btn-sm btn-warning"><i class="fas fa-fw fa-edit"></i></button>
                                <button class="btn btn-sm btn-danger"><i class="fas fa-fw fa-trash"></i></button>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row">1</th>
                            <td>2018-03-05 / 15:03:54</td>
                            <td> <a href="http://">201803/MAS/12-001</a> </td>
                            <td>DGT</td>
                            <td>Diterima Resepsionis</td>
                            <td>
                                <button class="btn btn-sm btn-primary"><i class="fas fa-fw fa-folder-open"></i></button>
                                <button class="btn btn-sm btn-danger"><i class="fas fa-fw fa-trash"></i></button>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row">1</th>
                            <td>2018-03-05 / 15:03:54</td>
                            <td> <a href="http://">201803/MAS/12-001</a> </td>
                            <td>DGT</td>
                            <td>Diterima Resepsionis</td>
                            <td>
                                <button class="btn btn-sm btn-primary"><i class="fas fa-fw fa-folder-open"></i></button>
                                <button class="btn btn-sm btn-warning"><i class="fas fa-fw fa-edit"></i></button>
                                <button class="btn btn-sm btn-danger"><i class="fas fa-fw fa-trash"></i></button>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row">1</th>
                            <td>2018-03-05 / 15:03:54</td>
                            <td> <a href="http://">201803/MAS/12-001</a> </td>
                            <td>DGT</td>
                            <td>Diterima Resepsionis</td>
                            <td>
                                <button class="btn btn-sm btn-primary"><i class="fas fa-fw fa-folder-open"></i></button>
                                <button class="btn btn-sm btn-warning"><i class="fas fa-fw fa-edit"></i></button>
                                <button class="btn btn-sm btn-danger"><i class="fas fa-fw fa-trash"></i></button>
                            </td>
                        </tr>

                    </tbody>
                </table>
            </div>
        </div>
    </div>
</div>
<!-- /.container-fluid -->

</div>
<!-- End of Main Content -->

<script>
    $(function() {
        $("#datepicker").datepicker();
    });
</script>